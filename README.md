...
Warning: `STDOUT_LOG_LEVEL` set the log level for birdwatcher-rs and all its dependency (mainly the telemetry SDK).
If you want to enable `debug` or `trace` with `STDOUT_LOG_LEVEL=debug`/`trace`, you will be submerged by logs not generated by birdwatcher it-self, but its dependencies.
If you only want to control the log of `birdwatcher-rs`, use this directive `STDOUT_LOG_LEVEL=birdwatcher_daemon=debug`
